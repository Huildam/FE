        ...
        <div>
          ...
          {step >= 2 && email.trim() !== "" && (
            <div style={{ padding: "0 12px" }}>
              ...
              >
                <label htmlFor="do" style={{ fontWeight: 600, fontSize: 18 }}>
                  지역(도/특별시)을 선택하세요
                </label>
                <Select
                  value={selectedDo}
                ...
                >
                  ...
                  <SelectContent>
                    {Object.keys(regions).map((doName: string) => (
                      <SelectItem key={doName} value={doName}>
                        {doName}
                      </SelectItem>
                    ))}
                  </SelectContent>
          ...
          {step >= 3 && selectedDo && (
            <div style={{ padding: "0 12px" }}>
              ...
              >
                <label htmlFor="si" style={{ fontWeight: 600, fontSize: 18 }}>
                  {selectedDo}의 시/구를 선택하세요
                </label>
                <Select
                  value={selectedSi}
                  onValueChange={(value) => setSelectedSi(value)}
                >
                  <SelectTrigger className="w-full mt-3">
                    <SelectValue placeholder="선택하세요" />
                  </SelectTrigger>
                  <SelectContent>
                    {(regions[selectedDo as keyof typeof regions] || []).map(
                      (siName: string) => (
                        <SelectItem key={siName} value={siName}>
                          {siName}
                        </SelectItem>
                      )
                    )}
